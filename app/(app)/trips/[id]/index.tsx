    ...
    .map((cat: any) => {
      ...
      if (selectedBag) {
        items = items.filter((item: any) =>
          item.bagIds?.includes(selectedBag)
        );
      }
      return { ...cat, data: items };
    })
    .filter((cat: any) => cat.data.length > 0 || !selectedBag);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        style={styles.bagTabs}
        contentContainerStyle={styles.bagTabsContent}
      >
        <Pressable
          style={[styles.bagTab, !selectedBag && styles.bagTabActive]}
          onPress={() => setSelectedBag(null)}
        >
          <Text
            style={[
              styles.bagTabText,
              !selectedBag && styles.bagTabTextActive,
            ]}
          >
            All
          </Text>
        ...
        {(bags || []).map((bag: any) => (
          <Pressable
            ...
            style={[
              styles.bagTab,
              selectedBag === bag._id && styles.bagTabActive,
              { borderColor: bag.color },
            ]}
            onPress={() =>
              setSelectedBag(selectedBag === bag._id ? null : bag._id)
            }
          >
            <View
              style={[styles.bagDot, { backgroundColor: bag.color }]}
            />
            <Text
              style={[
                styles.bagTabText,
                selectedBag === bag._id && styles.bagTabTextActive,
              ]}
            >
              {bag.name}
            </Text>
          ...
          <View style={styles.fabMenu}>
            ...
            >
              <Ionicons
                ...
                color={Colors.white}
              />
              <Text style={styles.fabMenuText}>Manage Bags</Text>
            </Pressable>
            <Pressable
              style={styles.fabMenuItem}
              onPress={() => {
